...
##2017
mpa17_protected <- MPA17 %>%
  filter(PROTECTED == "MPA")
      
mpa17_unprotected <- MPA17 %>%
  filter(PROTECTED == "Non-MPA")
...
ivee12 <- mpa12_protected %>%
  filter(SITE == "IVEE")

ivee17 <- mpa17_protected %>%
  filter(SITE == "IVEE")

ivee_t <- t.test(ivee12$SIZE, ivee17$SIZE)
...
MPA17_summary <- lobstar_abs_tidy %>% 
  filter(YEAR == "2017") %>% 
  mutate(PROTECTED = case_when(
    SITE == "IVEE" ~ "MPA",
    SITE == "NAPL" ~ "MPA",
...
MPA17_chi <- chisq.test(MPA17_summary$LEGAL_LOBSTARS)
MPA17_chi
