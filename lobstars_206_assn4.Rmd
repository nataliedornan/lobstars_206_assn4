...
lobstah_boxplot <- filter(lobstah_body, YEAR == "2017") %>%
  ggplot(aes(x = SITE, y = SIZE)) + 
  geom_boxplot(aes(color = SITE), width = 0.35) +
  labs(x = "Site", y = "Carapace Length (mm)") +
  theme_classic() +
  theme(legend.position = "none") +
  ggtitle("Carapice Length of Lobsters at Different Sites in the SB Channel")


## Levenes test for varience
...
penguin_aov <- aov(SIZE ~ SITE, data = lobstah_body)
...
MPA_Graph_12 <- ggplot(MPA12, aes(x= SITE, y= SIZE))+
  geom_boxplot(aes(color = PROTECTED))+
  theme_classic()+
  ylab("Carapice Length")


MPA_Graph_17 <- ggplot(MPA17, aes(x= SITE, y= SIZE))+
  geom_boxplot(aes(color = PROTECTED))+
  theme_classic()+
  ylab("Carapice Length")


MPA_Graph_12; MPA_Graph_17
```
